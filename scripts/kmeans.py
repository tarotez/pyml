...
def initializeClusters(samples, clusterNum):
    ...
    for clusterID in range(clusterNum):
        clusterCenters.append(
            np.array(
                [
                    np.random.uniform(minVal, maxVal)
                    for minVal, maxVal in zip(minVec, maxVec)
                ]
            )
        )
...
def assignToClosest(samples, clusterCenters):
    ...
    for sample in samples:
        closestClusterID = np.argmin(
            [np.linalg.norm(sample - clusterCenter) for clusterCenter in clusterCenters]
        )
...
def drawScatter(samples, assignments, clusterCenters, title):
    ...
    ax.scatter(
        clusterCentersArray[:, 0],
        clusterCentersArray[:, 1],
        edgecolors="red",
        c=[i for i in range(clusterNum)],
        s=300,
        lw=3,
    )
...
drawScatter(
    ...
    initial_assignments,
    clusterCenters,
    "Iteration 0, randomly placed clusters.",
)
for i in range(iterationNum):
    ...
    drawScatter(
        ...
        assignments,
        clusterCenters,
        "Iterattion " + str(i + 1) + ", after assignment of labels.",
    )
    ...
    drawScatter(
        ...
        assignments,
        clusterCenters,
        "Iteration " + str(i + 1) + ", after updating cluster locations.",
    )
